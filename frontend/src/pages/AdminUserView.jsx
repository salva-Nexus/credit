...
import { ArrowLeft, Save, CheckCircle, XCircle, Plus, RefreshCw, Pencil, Trash2 } from 'lucide-react';
import API from '../api';
...
export default function AdminUserView() {
  ...
  const [editingTxDate, setEditingTxDate] = useState(null);
  const [editDate, setEditDate] = useState('');
  const [savingDate, setSavingDate] = useState(false);
  const [confirmDeleteTx, setConfirmDeleteTx] = useState(null);
  const [refreshing, setRefreshing] = useState(false);
  const [deletingTx, setDeletingTx] = useState(false); // { [txId]: 'approve' | 'reject' }
  ...
  const handleSaveDate = async (txId) => {
    setSavingDate(true);
    try {
      await API.put(`/api/admin/transaction/${txId}/edit`, { date: new Date(editDate) });
  ...
  const handleDeleteTx = async (txId) => {
    setDeletingTx(true);
    try {
      await API.delete(`/api/admin/transaction/${txId}`);
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8fafc' }}>
      ...
      {confirmDeleteTx && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'white', borderRadius: 16, padding: 32, maxWidth: 380, width: '90%', boxShadow: '0 24px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ width: 52, height: 52, borderRadius: '50%', background: '#fef2f2', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px' }}>
              <Trash2 size={22} color="#dc2626" />
            </div>
            <h3 style={{ margin: '0 0 8px', fontSize: 17, fontWeight: 800, color: '#0f172a', textAlign: 'center' }}>Delete Transaction?</h3>
            <p style={{ margin: '0 0 24px', fontSize: 14, color: '#64748b', textAlign: 'center', lineHeight: 1.6 }}>
              This will permanently remove this transaction from the user's history. This cannot be undone.
            </p>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setConfirmDeleteTx(null)}
                style={{ flex: 1, padding: '11px', borderRadius: 9, border: '1.5px solid #e2e8f0', background: 'white', fontSize: 14, fontWeight: 600, cursor: 'pointer', color: '#374151' }}>
                Cancel
              </button>
              <button onClick={() => handleDeleteTx(confirmDeleteTx)} disabled={deletingTx}
                style={{ flex: 1, padding: '11px', borderRadius: 9, background: '#dc2626', color: 'white', border: 'none', fontSize: 14, fontWeight: 700, cursor: deletingTx ? 'not-allowed' : 'pointer', transition: 'background 0.15s', opacity: deletingTx ? 0.8 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
                onMouseEnter={e => { if (!deletingTx) e.currentTarget.style.background = '#b91c1c'; }}
                onMouseLeave={e => e.currentTarget.style.background = '#dc2626'}>
                {deletingTx ? <><div style={{ width: 14, height: 14, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} /> Deleting…</> : 'Delete'}
              </button>
            </div>
          </div>
        </div>
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc' }}>
      ...
      <div style={{ background: '#1a3c5e', padding: '20px 32px', display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10 }}>
          <button onClick={() => fetchUser()} disabled={refreshing}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px', borderRadius: 8, background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: 'white', fontSize: 12, cursor: refreshing ? 'not-allowed' : 'pointer', opacity: refreshing ? 0.7 : 1, transition: 'opacity 0.15s' }}>
            <RefreshCw size={12} style={{ animation: refreshing ? 'spin 0.7s linear infinite' : 'none' }} /> {refreshing ? 'Refreshing…' : 'Refresh'}
          </button>
          ...
          <div>
            ...
            <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden', overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 600 }}>
                <thead>
                  <tr>{['Type', 'Amount', 'Recipient', 'Date', 'Status', 'Actions'].map(h => <th key={h} style={thS}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {(user.transactions || []).length === 0 ? (
                    <tr><td colSpan={6} style={{ ...tdS, textAlign: 'center', color: '#94a3b8', padding: 40 }}>No transactions</td></tr>
                    ...
                    <tr key={tx._id} onMouseEnter={e => e.currentTarget.style.background = '#fafbff'} onMouseLeave={e => e.currentTarget.style.background = 'white'}>
                      <td style={tdS}><span style={{ fontSize: 12, fontWeight: 600, textTransform: 'capitalize', color: '#374151' }}>{tx.type?.replace(/_/g, ' ')}</span></td>
                      <td style={{ ...tdS, fontWeight: 700, fontSize: 14, color: tx.type === 'deposit' || tx.type === 'transfer_in' ? '#16a34a' : '#dc2626' }}>
                        {tx.type === 'deposit' || tx.type === 'transfer_in' ? '+' : '-'}${(tx.amount || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                      </td>
                      <td style={tdS}>
                        <p style={{ margin: 0, fontSize: 12 }}>{tx.recipientName || '—'}</p>
                        <p style={{ margin: 0, fontSize: 11, color: '#94a3b8' }}>{tx.memo || ''}</p>
                      </td>
                      <td style={tdS}>
                        {editingTxDate === tx._id ? (
                          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                            <input type="date" value={editDate} onChange={e => setEditDate(e.target.value)}
                              style={{ ...inp, padding: '5px 8px', fontSize: 12, width: 130 }} />
                            <button onClick={() => handleSaveDate(tx._id)} disabled={savingDate}
                              style={{ padding: '5px 10px', borderRadius: 6, background: '#1a3c5e', color: 'white', border: 'none', cursor: savingDate ? 'not-allowed' : 'pointer', fontSize: 11, fontWeight: 600, opacity: savingDate ? 0.7 : 1, display: 'flex', alignItems: 'center', gap: 5 }}>
                              {savingDate ? <><div style={{ width: 10, height: 10, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} /> Saving…</> : 'Save'}
                            </button>
                            <button onClick={() => setEditingTxDate(null)}
                              style={{ padding: '5px 8px', borderRadius: 6, border: '1px solid #e2e8f0', background: 'white', cursor: 'pointer', fontSize: 11 }}>✕</button>
                          </div>
                        ) : (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                            <span style={{ fontSize: 12, color: '#64748b' }}>{tx.date ? new Date(tx.date).toLocaleDateString('en-US', { dateStyle: 'medium' }) : '—'}</span>
                            <button onClick={() => { setEditingTxDate(tx._id); setEditDate(tx.date ? new Date(tx.date).toISOString().split('T')[0] : ''); }}
                              style={{ padding: '3px 6px', borderRadius: 5, border: '1px solid #e2e8f0', background: '#f8fafc', cursor: 'pointer', color: '#64748b', display: 'flex', alignItems: 'center' }}>
                              <Pencil size={11} />
                            </button>
                          </div>
                        )}
                      ...
                      <td style={tdS}>
                        <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 9px', borderRadius: 20, textTransform: 'capitalize',
                          background: tx.status === 'completed' ? '#f0fdf4' : tx.status === 'failed' ? '#fef2f2' : '#fffbeb',
                          color: tx.status === 'completed' ? '#16a34a' : tx.status === 'failed' ? '#dc2626' : '#d97706',
                          border: `1px solid ${tx.status === 'completed' ? '#bbf7d0' : tx.status === 'failed' ? '#fecaca' : '#fde68a'}` }}>
                          {tx.status}
                        </span>
                      </td>
                      <td style={tdS}>
                        <button onClick={() => setConfirmDeleteTx(tx._id)}
                          style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 10px', borderRadius: 7, border: '1px solid #fecaca', background: '#fef2f2', fontSize: 12, cursor: 'pointer', color: '#dc2626' }}>
